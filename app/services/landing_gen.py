...
def _render_persona_use_cases(cfg: BrandConfig) -> str:
    """4-card 'Built for every kind of merchant' grid · Shopify Solutions · R23 i18n."""
    return f'''
<section id="how-it-works" style="padding:80px 0;background:#FFFFFF">
  <div class="container">
    <div style="max-width:680px;margin:0 auto 48px;text-align:center">
      <div style="font-size:12px;color:var(--brand-dk);text-transform:uppercase;letter-spacing:1.4px;font-weight:800;margin-bottom:10px">{_t("personas.eyebrow", "Built for every kind of merchant")}</div>
      <h2 style="font-size:38px;font-weight:800;letter-spacing:-.8px;line-height:1.15;margin-bottom:14px;color:#0F172A">{_t("personas.title", "Pick the version of KiX that fits your shop.")}</h2>
      <p style="font-size:16px;color:#475569;line-height:1.55">{_t("personas.sub", "One product, four flavors. Same dashboard. Same games. Different proof, different price tier, different onboarding.")}</p>
    </div>
...
def _render_global_top_nav(cfg: BrandConfig) -> str:
    """Shopify-style sticky top nav unified across pages.
    R30 fix: data-i18n uses 'landing' namespace (already loaded) instead of
    'common' (was returning raw keys like 'nav.single'). Logo hard-coded (no i18n).
    """
    return f'''
<nav style="background:#fff;border-bottom:1px solid #E2E8F0;position:sticky;top:0;z-index:100;padding:0">
  <div class="container" style="display:flex;align-items:center;justify-content:space-between;padding:14px 24px">
    <div style="display:flex;align-items:center;gap:32px">
      <a href="/landing/brands/default/index.html" style="font-size:22px;font-weight:800;color:#0F172A;text-decoration:none">Ki<span style="color:var(--brand-dk)">X</span></a>
      <div class="nav-products" style="display:flex;gap:22px;font-size:14px;font-weight:600">
        <a href="/landing/brands/default/index.html" style="color:#475569;text-decoration:none" data-i18n="landing:nav.single">Single stall</a>
        <a href="/landing/brands/kopi_king_chain/index.html" style="color:#475569;text-decoration:none" data-i18n="landing:nav.chains">For chains</a>
        <a href="/landing/brands/kix_for_enterprise/index.html" style="color:#475569;text-decoration:none" data-i18n="landing:nav.enterprise">Enterprise</a>
        <a href="/landing/pricing.html" style="color:#475569;text-decoration:none" data-i18n="landing:nav.pricing">Pricing</a>
        <a href="/landing/brands/{_esc(cfg.brand_id)}/details.html" style="color:#475569;text-decoration:none" data-i18n="landing:nav.specs">Specs</a>
      </div>
    </div>
    <div style="display:flex;align-items:center;gap:14px">
      <span class="kix-lang-slot"></span>
      <a href="{_esc(cfg.portal_link)}" style="font-size:14px;color:#475569;text-decoration:none;font-weight:600" data-i18n="landing:nav.signin">Sign in</a>
      <a href="{_esc(cfg.portal_link)}?tier=free&brand={_esc(cfg.brand_id)}" style="background:#0F172A;color:#fff;padding:9px 18px;border-radius:7px;font-size:13.5px;font-weight:700;text-decoration:none" data-i18n="landing:nav.start_free">Start free</a>
    </div>
  </div>
  <style>@media(max-width:780px){{.nav-products{{display:none !important}}}}</style>
</nav>'''


def _render_faq_accordion(cfg: BrandConfig) -> str:
    """Shopify-style FAQ accordion — addresses last-mile objections."""
    return '''
<section id="faq" style="padding:80px 0;background:#F8FAFC;border-top:1px solid #E2E8F0">
  <div class="container">
    <div style="max-width:720px;margin:0 auto">
      <div style="text-align:center;margin-bottom:40px">
        <div style="font-size:12px;color:var(--brand-dk);text-transform:uppercase;letter-spacing:1.4px;font-weight:800;margin-bottom:10px" data-i18n="faq:tag">FAQ</div>
        <h2 style="font-size:36px;font-weight:800;letter-spacing:-.6px;margin-bottom:12px;color:#0F172A" data-i18n="faq:title">Questions buyers actually ask.</h2>
        <p style="font-size:15px;color:#64748B" data-i18n="faq:sub">Pulled verbatim from real buyer-journey simulations across 19 rounds.</p>
      </div>
      <style>details[open]{background:#fff;border-color:var(--brand)}details summary::-webkit-details-marker{display:none}details summary::after{content:"+";float:right;font-size:24px;color:#94A3B8;font-weight:300;line-height:1}details[open] summary::after{content:"−";color:var(--brand-dk)}</style>
      <details style="background:#fff;border:1px solid #E2E8F0;border-radius:10px;padding:18px 22px;margin-bottom:10px;cursor:pointer">
        <summary style="font-size:15.5px;font-weight:700;color:#0F172A;list-style:none;outline:none" data-i18n="faq:q1">Will my card be charged on the Free tier?</summary>
        <div style="padding-top:12px;color:#475569;font-size:14px;line-height:1.6" data-i18n="faq:a1"><strong>Never.</strong> Card-on-file is required as an anti-abuse signal — it filters out bots, hackers, and joke signups. We do not charge the card unless you explicitly upgrade. 1-click delete card + account anytime.</div>
      </details>
      <details style="background:#fff;border:1px solid #E2E8F0;border-radius:10px;padding:18px 22px;margin-bottom:10px;cursor:pointer">
        <summary style="font-size:15.5px;font-weight:700;color:#0F172A;list-style:none;outline:none" data-i18n="faq:q2">How fast do new customers actually walk in?</summary>
        <div style="padding-top:12px;color:#475569;font-size:14px;line-height:1.6" data-i18n="faq:a2">Bubble tea (Tea Trio composite): 423 new customers in month 1 at S$4.40 CPA. Kopitiam (Heng Heng): 47 new walk-ins per month at S$4.90 CPA. Halal hawker (Aminah): 23 in week 1. Your number depends on offer strength + foot traffic; see vertical benchmarks above.</div>
      </details>
      <details style="background:#fff;border:1px solid #E2E8F0;border-radius:10px;padding:18px 22px;margin-bottom:10px;cursor:pointer">
        <summary style="font-size:15.5px;font-weight:700;color:#0F172A;list-style:none;outline:none" data-i18n="faq:q3">What if I want to cancel?</summary>
        <div style="padding-top:12px;color:#475569;font-size:14px;line-height:1.6" data-i18n="faq:a3"><strong>1 click + 1 confirm.</strong> No retention email loop, no required reason. Settings → Plan → Cancel. Full 30-day data export + destruction certificate on request. We ship the export script in the same PR as your signup so it's never "we'll get to it later".</div>
      </details>
      <details style="background:#fff;border:1px solid #E2E8F0;border-radius:10px;padding:18px 22px;margin-bottom:10px;cursor:pointer">
        <summary style="font-size:15.5px;font-weight:700;color:#0F172A;list-style:none;outline:none" data-i18n="faq:q4">Does this work for my country / region / vertical?</summary>
        <div style="padding-top:12px;color:#475569;font-size:14px;line-height:1.6" data-i18n="faq:a4">5 markets live (SG, MY, HK, AU, ID). 3 waitlist (CN Shanghai/Shenzhen, IN, TH). 5 PSPs (PayNow, GrabPay, OVO, Alipay, WeChat). Bubble tea, kopi, halal hawker, café, gym, nail salon — all supported with vertical-specific benchmarks. Other verticals: case-by-case review.</div>
      </details>
      <details style="background:#fff;border:1px solid #E2E8F0;border-radius:10px;padding:18px 22px;margin-bottom:10px;cursor:pointer">
        <summary style="font-size:15.5px;font-weight:700;color:#0F172A;list-style:none;outline:none" data-i18n="faq:q5">My biggest competitor uses Capillary / Salesforce / Eber. Why switch?</summary>
        <div style="padding-top:12px;color:#475569;font-size:14px;line-height:1.6" data-i18n="faq:a5">You don't switch — you ADD KiX as the customer-acquisition layer. KiX flows verified-new-customer events INTO your existing loyalty CRM via Salesforce / Segment / mParticle / Adobe AEP / Snowflake bidirectional integrations. KiX brings them in; your existing loyalty platform keeps them. Best of both stacks.</div>
      </details>
      <details style="background:#fff;border:1px solid #E2E8F0;border-radius:10px;padding:18px 22px;cursor:pointer">
        <summary style="font-size:15.5px;font-weight:700;color:#0F172A;list-style:none;outline:none" data-i18n="faq:q6">Is the game library Halal-aware / family-friendly?</summary>
        <div style="padding-top:12px;color:#475569;font-size:14px;line-height:1.6" data-i18n="faq:a6">Yes — explicit Halal-aware library subset (no gambling-flavored mechanics, no impermissible imagery). Auto-applied for merchants tagged Halal. Family-friendly mode (no addictive streak loops, age-appropriate rewards) opt-in for any merchant. Audited per template at generation time.</div>
      </details>
    </div>
  </div>
</section>'''
...
def _render_comparison_table(cfg: BrandConfig) -> str:
    """Compare KiX vs alternative spend — Shopify "compare to other tools" pattern."""
    return '''
<section style="padding:80px 0;background:#FFFFFF">
  <div class="container">
    <div style="max-width:900px;margin:0 auto">
      <div style="text-align:center;margin-bottom:36px">
        <div style="font-size:12px;color:var(--brand-dk);text-transform:uppercase;letter-spacing:1.4px;font-weight:800;margin-bottom:10px" data-i18n="compare:tag">Compare vs your current spend</div>
        <h2 style="font-size:36px;font-weight:800;letter-spacing:-.6px;margin-bottom:12px;color:#0F172A" data-i18n="compare:title">Cheaper than IG ads. More accurate than flyers.</h2>
      </div>
      <div style="overflow-x:auto"><table style="width:100%;border-collapse:separate;border-spacing:0;font-size:14px;background:#fff;border:1px solid #E2E8F0;border-radius:12px;overflow:hidden">
        <thead><tr style="background:#F8FAFC">
          <th style="text-align:left;padding:14px 18px;font-size:11.5px;text-transform:uppercase;letter-spacing:.5px;color:#64748B;font-weight:800" data-i18n="compare:col_what">What you measure</th>
          <th style="text-align:center;padding:14px 18px;font-size:13px;color:#0F172A;font-weight:800;background:#F0FDF4;border-left:2px solid var(--brand)">KiX</th>
          <th style="text-align:center;padding:14px 18px;font-size:13px;color:#475569;font-weight:700">IG / FB boost</th>
          <th style="text-align:center;padding:14px 18px;font-size:13px;color:#475569;font-weight:700">Loyalty SaaS (Eber/Capillary)</th>
          <th style="text-align:center;padding:14px 18px;font-size:13px;color:#475569;font-weight:700">Print flyers</th>
        </tr></thead>
        <tbody>
          <tr><td style="padding:14px 18px;color:#0F172A;font-weight:600" data-i18n="compare:r1_label">Cost per VERIFIED new customer</td>
            <td style="padding:14px 18px;text-align:center;color:#14532D;font-weight:800;background:#F0FDF4">S$3-30 (vertical-banded)</td>
            <td style="padding:14px 18px;text-align:center;color:#475569">S$8-40 (no verify)</td>
            <td style="padding:14px 18px;text-align:center;color:#475569">S$50-200 (locked-in seats)</td>
            <td style="padding:14px 18px;text-align:center;color:#475569">Unknowable</td></tr>
          <tr style="background:#F8FAFC"><td style="padding:14px 18px;color:#0F172A;font-weight:600" data-i18n="compare:r2_label">Time to first new customer</td>
            <td style="padding:14px 18px;text-align:center;color:#14532D;font-weight:800;background:#F0FDF4">~5 min (live game)</td>
            <td style="padding:14px 18px;text-align:center;color:#475569">~hours (campaign approval)</td>
            <td style="padding:14px 18px;text-align:center;color:#475569">~weeks (onboarding)</td>
            <td style="padding:14px 18px;text-align:center;color:#475569">~days (print + distribute)</td></tr>
          <tr><td style="padding:14px 18px;color:#0F172A;font-weight:600" data-i18n="compare:r3_label">Returns customer in 14 days</td>
            <td style="padding:14px 18px;text-align:center;color:#14532D;font-weight:800;background:#F0FDF4">28% (cohort-tracked)</td>
            <td style="padding:14px 18px;text-align:center;color:#475569">Unknowable</td>
            <td style="padding:14px 18px;text-align:center;color:#475569">10-15% (point-balance only)</td>
            <td style="padding:14px 18px;text-align:center;color:#475569">Unknowable</td></tr>
          <tr style="background:#F8FAFC"><td style="padding:14px 18px;color:#0F172A;font-weight:600" data-i18n="compare:r4_label">Setup fee + commitment</td>
            <td style="padding:14px 18px;text-align:center;color:#14532D;font-weight:800;background:#F0FDF4">S$0 / cancel 1-click</td>
            <td style="padding:14px 18px;text-align:center;color:#475569">S$0 / pause anytime</td>
            <td style="padding:14px 18px;text-align:center;color:#475569">S$5K-50K setup / 1-3yr MSA</td>
            <td style="padding:14px 18px;text-align:center;color:#475569">~S$300/print run</td></tr>
          <tr><td style="padding:14px 18px;color:#0F172A;font-weight:600" data-i18n="compare:r5_label">Customer downloads an app?</td>
            <td style="padding:14px 18px;text-align:center;color:#14532D;font-weight:800;background:#F0FDF4">No · 3-sec QR</td>
            <td style="padding:14px 18px;text-align:center;color:#475569">No</td>
            <td style="padding:14px 18px;text-align:center;color:#475569">Usually yes</td>
            <td style="padding:14px 18px;text-align:center;color:#475569">No</td></tr>
        </tbody>
      </table></div>
      <div style="text-align:center;font-size:12.5px;color:#64748B;margin-top:18px">Numbers from public marketing-platform benchmarks (2024-2025) + 5 KiX SG alpha pilots. Your mileage varies; see vertical benchmarks above.</div>
    </div>
  </div>
</section>'''
...
def _render_final_cta_banner(cfg: BrandConfig) -> str:
    """Big bottom CTA — repeats the hero CTA. R23 i18n keys."""
    return f'''
<section style="padding:80px 0;background:linear-gradient(135deg,{_sanitize_hex(cfg.primary_color)} 0%,var(--brand-dk) 100%);color:#fff;text-align:center">
  <div class="container">
    <div style="max-width:680px;margin:0 auto">
      <h2 style="font-size:42px;font-weight:800;letter-spacing:-1px;line-height:1.1;margin-bottom:18px">{_t("final_cta.title1", "Start playing the game.")}<br><span style="opacity:.85">{_t("final_cta.title2", "Watch your shop fill up.")}</span></h2>
      <p style="font-size:17px;opacity:.9;margin-bottom:30px;line-height:1.55">{_t("final_cta.sub", "5 minutes to your first live game. Card on file (never charged on Free). Cancel 1-click. No phone calls from sales unless you ask.")}</p>
      <a href="{_esc(cfg.portal_link)}?tier=free&brand={_esc(cfg.brand_id)}" style="display:inline-block;background:#fff;color:#0F172A;padding:17px 40px;border-radius:8px;font-weight:800;text-decoration:none;font-size:17px;box-shadow:0 8px 24px rgba(0,0,0,.15)">{_t("final_cta.button", "Start free trial →")}</a>
      <div style="margin-top:18px;font-size:13px;opacity:.75">{_t("final_cta.signin_prompt", "Already have an account?")} <a href="{_esc(cfg.portal_link)}" style="color:#fff;text-decoration:underline;font-weight:700">{_t("final_cta.signin_link", "Sign in")}</a></div>
    </div>
  </div>
</section>'''
...
def _render_shopify_value_props(cfg: BrandConfig) -> str:
    """3 emotional value props — no stats, just feelings. Shopify-style. R23 i18n keys."""
    return f'''
<section style="padding:64px 0;background:#FFFFFF">
  <div class="container">
    <div style="max-width:1000px;margin:0 auto;text-align:center">
      <h2 style="font-size:32px;font-weight:800;letter-spacing:-.5px;margin-bottom:14px;color:#0F172A">{_t("values.title", "Three things you'll feel in week 1.")}</h2>
      <p style="font-size:15px;color:#475569;margin-bottom:40px;max-width:600px;margin-left:auto;margin-right:auto">{_t("values.sub", "Not numbers. Feelings. Your shop. Your customers. Your bottom line.")}</p>
      <div style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:24px;text-align:left">
        ...
        <div class="sh-val">
          ...
          <div style="font-size:18px;font-weight:800;margin-bottom:8px;color:#0F172A">{_t("values.card1.title", "More walk-ins this week")}</div>
          <div style="font-size:14px;color:#475569;line-height:1.55">{_t("values.card1.body", "A spin game lights up at the stall next door. Someone plays. They win a free coffee at your place. They come in this afternoon. That's it.")}</div>
        </div>
        <div class="sh-val">
          <div style="font-size:44px;margin-bottom:14px">🔁</div>
          <div style="font-size:18px;font-weight:800;margin-bottom:8px;color:#0F172A">{_t("values.card2.title", "They come back next week")}</div>
          <div style="font-size:14px;color:#475569;line-height:1.55">{_t("values.card2.body", "A second game runs only for first-time customers (you decide). 28% come back within 2 weeks. You watch this in a dashboard, not a spreadsheet.")}</div>
        </div>
        <div class="sh-val">
          <div style="font-size:44px;margin-bottom:14px">💰</div>
          <div style="font-size:18px;font-weight:800;margin-bottom:8px;color:#0F172A">{_t("values.card3.title", "Less wasted ad spend")}</div>
          <div style="font-size:14px;color:#475569;line-height:1.55">{_t("values.card3.body", "Stop boosting IG posts to people who already eat at your shop. Pay only when a verified NEW customer walks through your door. Most merchants cut their IG/FB spend 35-50% in month 1.")}</div>
        </div>
      </div>
    </div>
  </div>
...
<body>
<!-- R30 CRITICAL FIX: old header+hero deleted (was double-rendering with R23 global_top_nav + R21 shopify_hero) -->
'''
    ...
    if cfg.brand_id == "compliance":
        ...
        cards = "\n".join(
            f'<a href="{_esc(p.artifact_url or "#")}" style="display:block;background:#fff;border:1px solid #E2E8F0;border-radius:10px;padding:18px 20px;text-decoration:none;color:inherit;transition:border-color .15s" onmouseover="this.style.borderColor=\'var(--brand-dk)\'" onmouseout="this.style.borderColor=\'#E2E8F0\'">'
            f'<div style="font-size:11px;color:var(--brand-dk);text-transform:uppercase;letter-spacing:.6px;font-weight:800;margin-bottom:6px">{_esc(p.claim_id)}</div>'
            ...
            f'<div style="font-size:11.5px;color:var(--brand-dk);margin-top:8px;font-weight:700">View artifact →</div>'
            f'</a>' for p in regulator_proofs
        )
        compliance_html = head + f'''
<section style="padding:64px 0 32px;background:linear-gradient(135deg,var(--brand-dk) 0%,#0F172A 100%);color:#fff">
...
<section style="padding:48px 0;background:#F8FAFC">
  <div class="container" style="max-width:1080px">
    <div style="text-align:center;margin-bottom:32px">
      <div style="font-size:12px;color:var(--brand-dk);text-transform:uppercase;letter-spacing:1.4px;font-weight:800;margin-bottom:10px">10 compliance artifacts · all present</div>
...
<section style="padding:48px 0;background:#FFFFFF;text-align:center">
  <div class="container" style="max-width:760px">
    ...
    <a href="javascript:void(0)" onclick="alert('Bookmark this page (Cmd-D / Ctrl-D) for your review file.')" style="background:var(--brand-dk);color:#fff;padding:13px 28px;border-radius:8px;font-weight:700;text-decoration:none;font-size:15px;display:inline-block">Bookmark for review file</a>
  </div>
</section>
