...
tutorials-module-raid = الغارة

tutorials-module-squad = الفريق

tutorials-module-territory = المنطقة

tutorials-module-gift_sending = إرسال الهدايا

tutorials-module-trading_post = مركز التداول

tutorials-module-group_reward = مكافأة المجموعة

tutorials-module-fcfs = أولوية الحضور

tutorials-module-limited_drop = العرض المحدود

tutorials-module-triggers = المحفزات
...
tutorials-step-navigate-vouchers = افتح "القسائم" في الشريط الجانبي لتكوين قوالب القسائم

tutorials-step-navigate-rules = افتح "القواعد" في الشريط الجانبي لتكوين قواعد الأحداث

tutorials-step-enable-module = تفعيل وحدة { $module_name }

tutorials-step-configure-module = تكوين { $module_name }: { $params_summary }
...
tutorials-step-test-action = لنحاكي "{ $event_name }" لاختبار القواعد

tutorials-step-celebrate = تم! إعدادك لـ "{ $recipe_name }" يعمل الآن.

conditions-blocker-supply_exhausted = تم استنفاد العرض لهذه الحملة بالكامل.

conditions-blocker-budget_exhausted = تم استنفاذ ميزانية هذه الحملة بالكامل.
...
conditions-blocker-action_prerequisites_unmet = لم تكتمل الإجراءات المطلوبة مسبقًا.
...
conditions-blocker-reservation_already_committed = تم استخدام الحجز بالفعل.

conditions-blocker-reservation_already_refunded = تم استرداد الحجز بالفعل.

conditions-blocker-reservation_expired = انتهت صلاحية الحجز؛ يرجى المحاولة مرة أخرى.

conditions-blocker-commit_contention = تعارض عالي في الالتزام؛ يرجى المحاولة مرة أخرى.

welcome_kit-item-table_stand-title = حامل الطاولة (A5، وجهين)

welcome_kit-item-table_stand-desc = حامل مكتبي A5 مع دعوة إلى الإجراء عبر رمز الاستجابة السريعة على كلا الوجهين.

welcome_kit-item-counter_standing-title = حامل العداد (A4)

welcome_kit-item-counter_standing-desc = عرض عمودي A4 للعداد أو منطقة الاستقبال.

welcome_kit-item-door_sticker-title = ملصق الباب (150 مم دائري)

welcome_kit-item-door_sticker-desc = ملصق ثابت للباب / النافذة يدعو المارة للفحص.

welcome_kit-item-social_poster-title = ملصق اجتماعي (1080×1080)

welcome_kit-item-social_poster-desc = ملصق مربع جاهز لـ Instagram، Facebook، TikTok.
...
welcome_kit-default-tagline = افحص للعب. اربح الجوائز.
...
recipe_generator-summary-recipe-includes = تتضمن الوصفة '{ $recipe_name }' { $module_count ->
        [one] وحدة واحدة
        *[other] { $module_count } وحدات
    }: { $module_list }، متصلة بواسطة { $rule_count ->
        [one] قاعدة واحدة
        *[other] { $rule_count } قواعد
    }.

recipe_generator-heuristic-fallback = (قالب إرشادي) تمت مطابقة الوحدات ذات الصلة والقواعد الافتراضية من الكلمات المفتاحية.
