...
整个 KiX 的构建遵循一个固定循环。但第一步**不是写代码，甚至不是写需求——是写一份完整的蓝图文档**：

```
写蓝图文档（目录结构、数据模型、API 设计、实施阶段、参考系统）
    ↓
交给 AI 执行（"按照本机的环境执行这个计划"）
    ↓
...
扩充代码修复 → 回到测试
    ↓
对比蓝图做完整性检查（"review 系统现在还缺什么"）
```

**蓝图文档 = 1342 行，写了 4 个模块的完整目录结构、5 套数据库 schema、5 组 API 端点、15 个 NE 模块的标准接口、4 阶段实施计划、docker-compose、环境变量。基于 CEO 讨论 + 三体迭代架构验证 + 18 个参考系统的研究。**

这个循环在 2 天内转了约 15 轮。

---

## 第〇轮（真正的起点）：写一份完整的蓝图文档

### 这不是一个"需求描述"。这是一份项目计划。

构建 KiX 的第一步不是写代码。是写了一篇 **1342 行、49KB 的完整项目计划**。

这份文档叫 `2026-05-23-kix-platform-project-plan.md`，包含：

- **架构总览** — KID/KIN/KASH/KLUB 四模块设计
- **完整目录结构** — 每个目录、每个文件、每个功能
- **技术栈选型** — 为什么选 Python、为什么统一生态
- **PostgreSQL 数据模型** — 四套完整 schema（身份、互动、商业、运营）
- **核心 API 设计** — 五组 REST 端点（身份、引擎、商业、品牌、竞赛）
- **KIN 模块标准接口** — ABC 抽象基类、事件进/状态出模式
- **事件系统** — Kafka/Redis Streams 连接所有模块
- **四阶段实施顺序** — Phase 1~4，每阶段明确的交付物
- **8 条铁律** — 开发硬约束，写在文档里
...
> **基于**: CEO 讨论后确认的改进报告 + 4 份架构研究报告

在写这份计划之前，用户（产品负责人）做了三件事：

**1. CEO 讨论 → 改进报告。**
不是"老板说要做一个游戏化平台"。是讨论完之后写了改进报告——明确了什么问题、什么方案、什么不做的（Kill List）。

**2. 三体迭代 Round 2 — 四维度架构验证。**
四个并行研究 Agent 分别深挖：身份架构、实时基础设施、经济设计、冷启动策略。每个维度都推翻了 Round 1 的建议，找到了更优方案：

| 维度 | R1 建议 | R2 验证结果 | 最终决策 |
|------|---------|-------------|---------|
| 身份 | CNAME/Redirect/S2S 三选一 | 三个都有致命缺陷 | Option D: SDK Token（Google Analytics 模型） |
| 基础设施 | Nakama Go plugin 热路径 | Nakama 本身是负担 | 彻底跳过 Nakama，自建 Go Service + Redis + Centrifugo |
| 经济 | 三层货币同时设计 | Phase 0 过度工程 | 两货币（Energy + Vouchers），Cash Phase 2 |
| 冷启动 | 先做单边工具 | 方向正确缺执行细节 | 10 周执行手册 + 7 组件 + Kill Threshold |
...
| 模块 | 参考系统 | 借鉴的模式 |
|------|---------|-----------|
| 身份 | Firebase Anonymous Auth | signInAnonymously → linkWithCredential |
| 互动 | Duolingo League | 30 人分组 + 10 级段位 + 周升降 |
| 商业 | Voucherify | Voucher 生命周期 |
| 运营 | 淘宝千牛 | 统一工作台，自助广告，插件市场 |
| SDK | Google Analytics | 一个 `<script>` 标签集成 |

### 这份文档的作用

它成了**整个构建过程的唯一真相源**。后续所有的迭代、gap 分析、代码实现，都回到这份文档来对照。
...
**把 1342 行的蓝图，交给 Claude Code 去执行。**

这是整个方法论最关键的一步：**先写完整的计划，再让 AI 执行。** 不是"先写个 MVP 看看"。不是"边做边想"。是先把每一个目录、每一个数据表、每一个 API 端点的名字都定下来，然后才开始写代码。
